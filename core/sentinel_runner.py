...
def _node_command_prefix(node_executable: str) -> list[str]:
    """
    返回可执行 Node 命令前缀。Node 16 需要显式打开实验 fetch，
    否则 sentinel-runner.js 的浏览器沙箱会报 ReferenceError: fetch is not defined。
    """
    probe = 'process.stdout.write(typeof fetch)'
    try:
        proc = subprocess.run(
        ...
        if proc.returncode == 0 and (proc.stdout or "").strip() == "function":
    ...
    try:
        proc = subprocess.run(
            [node_executable, "--no-warnings", "--experimental-fetch", "-e", probe],
            capture_output=True,
            text=True,
            timeout=5,
        )
        if proc.returncode == 0 and (proc.stdout or "").strip() == "function":
            return [node_executable, "--no-warnings", "--experimental-fetch"]
    except Exception:
        pass
    return [node_executable]
...
def generate_sentinel_token(
    ...
    try:
        ...
        cmd = [
            *_node_command_prefix(_resolve_node_executable()),
            str(_RUNNER_PATH),
